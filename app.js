...
app.get("/dashboard-data", async (req, res) => {
  try {
    ...
    for (const user of users) {
      ...
      const answeredPercentageAll = ((answered / totalQuestions) * 100).toFixed(
        2
      );
    ...
    const percentageCulture = ((skillCulture / categoryCulture) * 100).toFixed(
      2
    );
    const percentageHistory = ((skillHistory / categoryHistory) * 100).toFixed(
      2
    );
